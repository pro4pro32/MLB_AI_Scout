...
/* ── Dropdown/listbox popovers: force BOTH background AND text color
   together. BaseWeb's internal markup (the library behind Streamlit's
   selectbox/multiselect dropdowns) has changed its data-attributes
   across Streamlit versions; relying on a text-color rule alone risks
   light text landing on a light background if the background selector
   goes stale. This covers every variant seen across versions. ────── */
    ...
    for path in STATCAST_FILES[year]:
        ...
        try:
            if PYARROW_OK:
                ...
                pid_filter = float(pitcher_id) if pa.types.is_floating(pitcher_type) else int(pitcher_id)

# ═══════════════════════════════════════════════════════════════════
# AI.py  —  MLB Statcast Pro Dashboard
# FIXED VERSION — see changelog at bottom of chat response for details
# ═══════════════════════════════════════════════════════════════════
...
# ── 2. PAGE CONFIG ────────────────────────────────────────────────────
# FIX: set_page_config must be called exactly ONCE, and it must be the
# very first Streamlit command executed. The original file called it
# twice (once at the top, once again a few lines later), which raises
# StreamlitAPIException and crashes the app on every load.
st.set_page_config(
...
# NOTE: the original file also called st.cache_data.clear() unconditionally
# at import time. That wipes every @st.cache_data result on *every single
# rerun* (every widget click, every filter change), forcing the app to
# reload and reprocess all parquet files from scratch each time — with
# multi-year Statcast data this makes the app unusably slow / prone to
# timing out on Streamlit Cloud. Caching only works if you don't clear it
# constantly. A manual "Clear Cache" button already exists in the sidebar
# below for when you actually need to force a refresh after a redeploy,
# so the automatic clear() call has been removed.

# NOTE: the original file also defined an unused `load_minimal_data()`
# helper that called `load_pitching_stats(...)` before that function was
# defined anywhere in the module and was never invoked by any part of the
# app. It was dead code and has been removed.
...
# FIX: some seasons are shipped as ONE whole-year file (statcast_2024.parquet)
# instead of split monthly files (statcast_2024_03.parquet ... _10.parquet).
# Without this, a year with only the whole-year file present had an empty
# list here and got deleted below, so the app silently acted as if that
# season had zero data (no pitchers/batters selectable for it at all).
for year in [2024, 2025, 2026]:
    if not STATCAST_FILES.get(year):
        _whole_year_file = DATA_DIR / f"statcast_{year}.parquet"
        if _whole_year_file.exists():
            STATCAST_FILES[year] = [_whole_year_file]
...
PITCH_GROUP_LABELS = ["All"] + list(PITCH_GROUPS.keys())


def pitch_type_options_for_group(all_types: list, group: str) -> list:
    """Narrow a flat list of pitch-type codes to those in the selected
    Pitch Group (or return them all if group == 'All')."""
    if group == "All":
        return ["All"] + list(all_types)
    members = PITCH_GROUPS.get(group, [])
    return ["All"] + [pt for pt in all_types if pt in members]


def effective_pitch_filter(selected_type: str, selected_group: str):
    """
    Resolve what to actually filter pitch-level data by, combining the
    Pitch Group and Pitch Type selectors.

    FIX: previously, picking a Pitch Group only narrowed which codes the
    Pitch Type dropdown *offered* — if Pitch Type was left on 'All', the
    group selection had no effect at all on the underlying heatmap/arsenal
    data (e.g. choosing 'Fastballs' + 'All' still analyzed every pitch
    type). Now, when Pitch Type is 'All' but a specific group is chosen,
    this returns the group's full list of codes so callers can filter by
    `.isin(...)`. Returns "All" (no filtering), a single code string, or
    ...
    if selected_group == "All":
        return "All"
    return PITCH_GROUPS.get(selected_group, [])


def _apply_pitch_type_filter(df: pd.DataFrame, pitch_type) -> pd.DataFrame:
    """Filter a DataFrame's 'pitch_type' column by a value that may be
    'All', a single code string, or a list of code strings."""
    if pitch_type is None or pitch_type == "All":
        return df
    codes = pitch_type if isinstance(pitch_type, (list, tuple, set)) else [pitch_type]
    return df[df["pitch_type"].isin(codes)]
...
/* ── Dropdown/listbox popovers (selectbox, multiselect option lists).
   FIX: earlier attempts forced a dark background (#111621) here, but
   that isn't reliably applying in every Streamlit version — the
   popover keeps rendering with its native light background. Forcing
   near-white text on top of that made it unreadable. Instead of
   fighting the background, force strongly dark/black text so it's
   readable regardless of which background actually renders. ────── */
...
def apply_tab1_filters(
    ...
    dff = _apply_pitch_type_filter(dff, pitch_type)
...
def build_pitcher_arsenal(df: pd.DataFrame) -> tuple:
    ...
    arm_avg = safe_num(df["arm_angle"].mean())      if "arm_angle"         in df.columns else np.nan
    ext_avg = safe_num(df["release_extension"].mean()) if "release_extension" in df.columns else np.nan
...
def generate_pitching_plan(batter_stats: dict, batter_name: str) -> str:
    ...
    if not cnt.empty:
        ...
        for count, advice in [
            ...
            row = cnt_map.loc[count] if count in cnt_map.index else None
            extra = ""
            if row is not None:
                sw_pct  = safe_num(row["swing_pct"])
                wh_pct  = safe_num(row["whiff_pct"])
...
# ═══════════════════════════════════════════════════════════════════
# TAB 1 HELPERS
# FIX: the original file defined load_raw_for_subzones,
# compute_zone_stats_with_movement, draw_subzone_panel and
# draw_all_subzones_grid TWO-TO-THREE times each, with earlier copies
# containing a real bug (treating STATCAST_FILES[year], which is a
# *list* of monthly file paths, as if it were a single Path — calling
# `.exists()` on a list raises AttributeError). Only the last
# definition of each (which correctly loops over the monthly files)
# is kept here, once, to avoid that crash and the wasted duplication.
# ═══════════════════════════════════════════════════════════════════
...
def load_raw_for_subzones(
    ...
    Loops over each year's monthly parquet files (STATCAST_FILES[yr] is a
    list of Paths), applying pyarrow predicate pushdown where possible.
    """
    RAW_COLS = [
        "pitch_type","zone","stand","p_throws","balls","strikes",
        ...
        for path in STATCAST_FILES[yr]:
            ...
            try:
                if PYARROW_OK:
                    ...
                    filters = []
                    if pitch_type != "All":
                        if isinstance(pitch_type, (list, tuple, set)):
                            filters.append(("pitch_type", "in", list(pitch_type)))
                        else:
                    ...
                    df = _apply_pitch_type_filter(df, pitch_type)
                    if p_throws != "All":
...
with tab_main:
    ...
    _pt_opts_raw = sorted(_zone_df["pitch_type"].dropna().unique().tolist())
    _pt_opts = ["All"] + _pt_opts_raw
    ...
    r1 = st.columns([1,1,1,1,1,1,2])
    with r1[0]: yr_m = st.multiselect("Season", _yr_opts, default=_yr_opts, key="m_yr")
    with r1[1]: pg_m = st.selectbox("Pitch Group", PITCH_GROUP_LABELS, key="m_pg")
    with r1[2]:
        pt_m = st.selectbox("Pitch Type", pitch_type_options_for_group(_pt_opts_raw, pg_m), key="m_pt")
    with r1[3]: ph_m = st.selectbox("Pitcher Hand", ["All","R","L"], key="m_ph")
    with r1[4]: bh_m = st.selectbox("Batter Hand", ["All","R","L"], key="m_bh")
    with r1[5]: cnt_m = st.selectbox("Count", ["All"]+ALL_COUNTS, key="m_cnt")
    with r1[6]: stat_m = st.selectbox("Statistic", STAT_LABELS, key="m_stat")

    _eff_pt_m = effective_pitch_filter(pt_m, pg_m)
    ...
    if _has_mvmt:
        with st.spinner("Applying movement filters..."):
            _raw_m = load_raw_for_subzones(
                years=tuple(yr_m), pitch_type=_eff_pt_m, p_throws=ph_m,
        ...
        _df_main = apply_tab1_filters(_zone_df, yr_m, _eff_pt_m, ph_m, bh_m, cnt_m)
    ...
    # FIX: the original code here had a stray dedent that turned the
    # "compute movement columns if missing" block into a *separate*
    # top-level `if`, sitting between the block that defines
    # `_sum_base`/`_sum_mvmt` and the block that uses them. When
    # `_df_main` was empty, `_sum_mvmt` was never created, so the
    # dedented `if not _sum_mvmt and ...` line raised a NameError and
    # crashed the whole page. Everything below is now correctly nested
    # inside a single `if not _df_main.empty: / else:`.
    st.markdown(
    ...
    if not _df_main.empty:
        ...
        # If movement cols aren't already in _df_main (pre-agg path), compute
        # them from the raw sub-zone data we already loaded above.
        if not _sum_mvmt and not _raw_for_sz.empty:
            with st.spinner("Computing movement stats..."):
                _raw_for_mvmt = _add_flags(_raw_for_sz.copy())
    ...
    for col_w, pfx, default_stat, lbl in [
        ...
        with col_w:
            ...
            xc0, xc1, xc2, xc3 = st.columns(4)
            with xc0: pg_x  = st.selectbox("Pitch Group", PITCH_GROUP_LABELS, key=f"{pfx}_pg")
            with xc1: pt_x  = st.selectbox("Pitch Type", pitch_type_options_for_group(_pt_opts_raw, pg_x), key=f"{pfx}_pt")
            ...
            _eff_pt_x = effective_pitch_filter(pt_x, pg_x)

            _df_x = apply_tab1_filters(
                _zone_df,
                years=yr_m, pitch_type=_eff_pt_x,
                p_throws=ph_x, stand=bh_x, count_state=cnt_x,
            )
            _ttl_x = st_x
            if pt_x  != "All": _ttl_x += f"  ·  {pt_x}"
            elif pg_x != "All": _ttl_x += f"  ·  {pg_x}"
                ...
                if not _show_sz_cmp:
                    ...
                    with st.spinner(f"Sub-zone data {lbl}…"):
                        _raw_cmp = load_raw_for_subzones(
                            years=tuple(yr_m), pitch_type=_eff_pt_x, p_throws=ph_x,
...
def build_pitcher_team_roster(pitching_df: pd.DataFrame,
    ...
    # FIX: Baseball-Reference roster pulls sometimes include players with
    # no actual MLB innings (blank/0 IP — e.g. 40-man roster/taxi-squad
    # entries). Require real innings so they don't clutter the pitcher
    # search with players who never actually pitched in the majors.
    if "IP" in pitching_df.columns:
        pitching_df = pitching_df[pd.to_numeric(pitching_df["IP"], errors="coerce") > 0]
    ...
    # FIX: Baseball-Reference's season stat line for a pitcher usually
    # lists only ONE team (often just the team he ended the season with,
    # or whichever row happens to be in the file), so a pitcher traded
    # mid-season could be invisible under his other team(s) even though
    # Statcast has him correctly tagged for each team he actually pitched
    # for. meta_pitchers.parquet already has one row per
    # (pitcher_id, season, team), so fold those in directly too —
    # a pitcher then shows up under every team he really threw for.
    meta_p_path = DATA_DIR / "meta_pitchers.parquet"
    if meta_p_path.exists():
        try:
            mp = pd.read_parquet(meta_p_path, engine="pyarrow")
            for _, row in mp.iterrows():
                season = int(row["season"])
                team   = str(row.get("team", "?"))
                hand   = str(row.get("hand", "?"))
                disp_name = str(row.get("display_name") or "").strip()
                if not disp_name:
                    continue
                disp_key = f"{disp_name}  ({'RHP' if hand=='R' else 'LHP'})"
                roster.setdefault(season, {}).setdefault(team, [])
                if disp_key not in roster[season][team]:
                    roster[season][team].append(disp_key)
        except Exception:
            pass
...
def generate_detailed_matchup_plan(
    ...
    if not opp:
        html.append(
            ...
            f'Slider/sweeper runs away from the batter — '
...
with tab_pitcher:
    ...
    if not _ps_raw.empty and _ps_pid is not None:
        ...
        st.pyplot(fig_ars, width="stretch")
        plt.close(fig_ars)
        ...
        with fcols[3]:
            ...
            hz_pg = st.selectbox("Pitch Group", PITCH_GROUP_LABELS, key="ps_hpg2")
        with fcols[4]:
            hz_pt = st.selectbox("Pitch Type", pitch_type_options_for_group(_pt_opts_p, hz_pg), key="ps_hpt2")
        with fcols[5]:
        ...
        elif hz_pg != "All": _dff = _apply_pitch_type_filter(_dff, PITCH_GROUPS.get(hz_pg, []))
        ...
        elif hz_pg != "All": _hz_title += f" · {hz_pg}"
        ...
        with hz_col1:
            if not _hz_zdf.empty:
                ...
                st.pyplot(fig_hz, width="stretch")
                plt.close(fig_hz)
            else:
        ...
        _pitcher_report_html = generate_pitcher_report(
            _ps_name, _ars, _total, _arm_avg, _ext_avg, _hand,
            f"{_ps_year} Season",
        )
        st.markdown(_pitcher_report_html, unsafe_allow_html=True)

        _rep_txt = re.sub(r"<[^>]+>","", _pitcher_report_html).replace("&amp;","&")
        st.download_button(
        ...
        st.caption(
            ...
            "Generates a detailed attack plan using pitcher's arsenal "
                ...
                with st.expander(f"⚔️  {vb_pname} vs {bname}", expanded=True):
                    ...
                    with ov1:
                        ...
                        if not _bz_vb.empty:
                            ...
                            st.pyplot(fig_bz_vb, width="stretch")
                            plt.close(fig_bz_vb)
                        else:
...
with tab_batter:
    ...
    if not _bs_raw.empty and _bs_bid is not None:
        ...
        bf1, bf2, bf3, bf4, bf5 = st.columns([1, 1, 1, 1, 2])
        with bf1:
        ...
        with bf3:
            ...
            bs_pg = st.selectbox("Pitch Group", PITCH_GROUP_LABELS, key="bs_pg2")
        with bf4:
            bs_pt = st.selectbox(
                "Pitch Type",
                pitch_type_options_for_group(_pt_opts_b, bs_pg),
                key="bs_pt2",
            )
        with bf5:
        ...
        elif bs_pg != "All": _bff = _apply_pitch_type_filter(_bff, PITCH_GROUPS.get(bs_pg, []))
        ...
        if not _bstats:
            ...
            with col_hm_b:
                ...
                elif bs_pg != "All": _bs_title += f" · {bs_pg}"
                fig_bs_hm = draw_heatmap(
                    _bzdf, bs_stat, _bs_title,
                    batter_mode=True,
                )
                st.pyplot(fig_bs_hm, width="stretch")
                plt.close(fig_bs_hm)
            ...
            if not _bpt.empty:
                ...
                # FIX: this bar chart block used to sit at the wrong (shallower)
                # indentation level, outside the `if not _bstats:`/`else:` that
                # defines `_bpt`. Whenever a batter had no data for the current
                # filters (the `if not _bstats:` warning branch), Python still
                # tried to run this code and crashed with
                # NameError: name '_bpt' is not defined. It is now correctly
                # nested inside the `if not _bpt.empty:` block above, right
                # where the pitch-type table is rendered.
                if "whiff_pct" in _bpt.columns:
                    ...
                    if not _bpt_sorted.empty:
                        ...
                        st.pyplot(fig_pt, width="stretch")
                        plt.close(fig_pt)
            else:
            ...
            if not _bcnt.empty:
                ...
                plt.tight_layout()
                st.pyplot(fig_cnt, width="stretch")
                plt.close(fig_cnt)
            ...
            st.markdown(f'<div class="sec-hdr">🤖 AI Pitching Plan — How to Attack {_bs_name}</div>',
                        unsafe_allow_html=True)
